...
- Channels: 
    - Individual inputs: absence, mental health
    - Parental inputs: job loss, financial constraints, mental health, migration.
    - Infrastructure: School closures, breakdown transit system, teachers.
- Student achievement matters for individual (labor market) returns, human capital, and economic growth.
...
where $Disaster_{i,t}$ is the number of natural disasters in the past 5 years in a county and year, $\alpha_i$  and $\lambda_t$ are county and year fixed effects, $\mathbf{X}_i \lambda_t$ includes an interaction between initial county population and years. 
...
- Setting: Number and consts of natural disasters have been increasing.
...
- However, parts of the effect could be explained by migration. 
...
Coffee? `r fontawesome::fa('mug-hot')`
...
- Literature on the effect natural disaster in developing countries focuses on educational investment (eg. Deuchert and Felfe 2015)
...
- Reduced instuction time due to snow (Marcotte 2006, Goodman 2006, Marcotte and Hemelt 2008) `r fontawesome::fa('long-arrow-alt-right')` channel of reduced instruction time to narrow in this setting
...
knitr::include_graphics("index/sunab_fema_enrol.png") 
```
]
...
.center[
```{r out.width = '45%' , echo=FALSE}
knitr::include_graphics("index/sunab_fema_ppexp.png") 
```
]
